...
const renderCard = (card, router) => (
  ...
  >
    <img src={card.image} className="w-[278px] h-[338px] rounded-[10px] object-cover" />
    <div className="flex flex-col gap-4 justify-between w-full p-[14px]">
      <div>
        <button className="bg-[#2C2C2C] py-1 px-3 rounded text-sm text-white font-kraeftig">{card.category}</button>
      ...
      <div className="flex justify-between items-start mt-4">
        ...
        <button className="bg-activegreen py-2 px-4 rounded-[10px] text-black text-xs font-kraeftig mt-4">{card.buttonText}</button>
      </div>
    </div>
  </div>
);
